// test.js — Playwright-скрипт для тестирования лендинга и пэйвола
// Запуск: node test.js
...
const logFile = path.join(logsDir, logDate + '_' + urlSlug + '.log');
const logStream = fs.createWriteStream(logFile, { flags: 'a' });
logStream.write('=== LandingTester · ' + new Date().toLocaleString('ru') + ' ===\n');
logStream.write('URL: ' + config.landingUrl + '\n');
logStream.write('Device: ' + (config.device || 'chromium') + '\n\n');
...
async function runTests() {
  ...
  const deviceMap = {
    'chromium': { br: chromium, opts: { headless: false, slowMo: config.slowMo||400, args: ['--start-maximized'] }, ctx: {} },
    'yandex':   { br: chromium, opts: { headless: false, slowMo: config.slowMo||400, executablePath: 'C:/Users/' + (process.env.USERNAME||'user') + '/AppData/Local/Yandex/YandexBrowser/Application/browser.exe', args: ['--start-maximized'] }, ctx: {} },
    'iphone':   { br: webkit,   opts: { headless: false, slowMo: config.slowMo||400 }, ctx: devices['iPhone 13'] },
    'pixel':    { br: chromium, opts: { headless: false, slowMo: config.slowMo||400 }, ctx: devices['Pixel 5'] },
  };
  const dev = deviceMap[config.device||'chromium'] || deviceMap['chromium'];
